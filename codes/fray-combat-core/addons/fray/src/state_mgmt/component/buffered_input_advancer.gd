...
## 丢弃缓冲中指定名称的全部 sequence，并返回实际删除数量。
func discard_sequence(sequence_name: StringName) -> int:
	var removed_count := 0
	for index in range(_input_buffer.size() - 1, -1, -1):
		var buffered_input := _input_buffer[index] as BufferedInputSequence
		if buffered_input != null and buffered_input.sequence_name == sequence_name:
			_input_buffer.remove_at(index)
			removed_count += 1
