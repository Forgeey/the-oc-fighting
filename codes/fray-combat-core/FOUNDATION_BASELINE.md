...
> **历史快照：** 本文记录第 1 组独立 Fighter Foundation 项目的设计与当时目录。当前 `fray-combat-core` 项目不再保留 `fighter_foundation.tscn`、Foundation 调试覆盖层及单角色控制脚本；基础 `FoundationFighter`、Fray 状态机、输入、移动和攻击资源仍由 `CombatCoreFighter` 复用。当前唯一具体 Fighter 场景是 `res://scenes/combat_core_fighter.tscn`。
...
| `;` | 保持防御；软倒地受身窗口中重新按下可进入 `tech_roll` |
...
朝向由 `FoundationFighter.facing_direction` 直接保存。角色与 `OpponentMarker2D` 换边时，fighter 在允许自动改向的地面状态中直接更新朝向；角色图片与 Fray 攻击判定管理器随之镜像，同一物理方向键随后会映射为新的 `forward` / `back` 语义。
...
- 朝向改变会发出 `facing_changed` 信号；表现层以当前下蹲图片序列播放独立 `turn` 动画，而不阻塞 Fray 战斗状态。
