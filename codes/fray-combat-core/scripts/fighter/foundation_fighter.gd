...
const MOVE_DEFINITIONS := {
	...
	&"stand_medium": {"name": "站立中攻击", "command": "K"},
	&"stand_heavy": {"name": "站立重攻击", "command": "L"},
	&"stand_combo_medium": {"name": "轻接中组合技", "command": "J → K"},
	&"crouch_light": {"name": "下蹲轻攻击", "command": "2J"},
	&"crouch_medium": {"name": "下蹲中攻击", "command": "2K"},
	&"crouch_heavy": {"name": "下蹲重攻击", "command": "2L"},
	&"air_light": {"name": "空中轻攻击", "command": "空中 J"},
	&"air_medium": {"name": "空中中攻击", "command": "空中 K"},
	&"air_heavy": {"name": "空中重攻击", "command": "空中 L"},
	&"projectile": {"name": "波动攻击", "command": "4, 6, J"},
	&"enhanced_projectile": {"name": "强化波动攻击", "command": "4, 6, J → ;（启动窗口）"},
...
## 本角色使用的 Fray/Godot 输入名前缀，允许同一场景装配两名 fighter。
@export var input_prefix: StringName = &"p1_"
...
@onready var _attack_hit_state_manager: FrayHitStateManager2D = find_child("AttackHitStateManager", true, false) as FrayHitStateManager2D
@onready var _hurt_hit_state: FrayHitState2D = find_child("HurtState", true, false) as FrayHitState2D
...
func initialize_fighter() -> bool:
	...
	_sync_facing_visual()
	if not _input_setup.configure_input_map(input_prefix, self):
		return false
	var sequence_tree := _input_setup.create_combat_sequences(input_prefix)
	...
	var context := {
		...
		"input_prefix": _input_setup.get_prefix(),
	}
...
## 返回已由 FrayHitStateManager2D 校验的攻击状态 ID，供状态机按资源生成取消转移。
func get_attack_state_ids() -> Array[StringName]:
	var result: Array[StringName] = []
	for state_id_value in _attack_attributes.keys():
		result.append(StringName(state_id_value))
	result.sort()
	return result
...
## 返回状态机、运动和输入是否正由 hitstop 等外部流程集中暂停。
...
func request_external_state(kind: StringName, args: Dictionary = {}) -> bool:
	...
	match kind:
		&"hitstun":
			...
			state_machine.goto(target, {
				"attribute": reaction_attribute,
				"combo_air_hits": int(args.get("combo_air_hits", 0)),
				"defender_was_airborne": bool(args.get("defender_was_airborne", false)),
			})
			return true
		&"blockstun":
			reaction_attribute = resolve_reaction_attribute(args.get("attribute", debug_reaction_attribute))
...
func _validate_attack_hit_state(
	...
	for hitbox in hitboxes:
		...
		result = attack
	if (
		result.cancel_requirement < FrayAttackAttribute.CancelRequirement.ALWAYS
		or result.cancel_requirement > FrayAttackAttribute.CancelRequirement.ON_HIT_OR_BLOCK
	):
		push_error("攻击状态 %s 的 cancel_requirement 无效。" % state_id)
		return null
	if result.has_cancel_window():
		if result.cancel_start_frame >= maxi(result.duration_frames, 1):
			push_error("攻击状态 %s 的 cancel_start_frame 必须早于攻击结束。" % state_id)
			return null
		if result.cancel_end_frame >= 0 and result.cancel_end_frame < result.cancel_start_frame:
			push_error("攻击状态 %s 的 cancel_end_frame 不能早于 cancel_start_frame。" % state_id)
			return null
		if result.cancel_end_frame >= maxi(result.duration_frames, 1):
			push_error("攻击状态 %s 的 cancel_end_frame 必须早于攻击结束。" % state_id)
			return null
	var cancel_targets := result.cancel_target_states
	if not cancel_targets.is_empty() and not result.has_cancel_window():
		push_error("攻击状态 %s 配置了取消目标，但没有配置 cancel window。" % state_id)
		return null
	if cancel_targets.is_empty() and result.has_cancel_window():
		push_error("攻击状态 %s 配置了 cancel window，但没有 cancel_target_states。" % state_id)
		return null
	var unique_targets: Dictionary = {}
	for target_value in cancel_targets:
		var target_state := StringName(target_value)
		if target_state.is_empty() or target_state == state_id:
			push_error("攻击状态 %s 包含无效取消目标 %s。" % [state_id, target_state])
			return null
		if unique_targets.has(target_state):
			push_error("攻击状态 %s 重复配置取消目标 %s。" % [state_id, target_state])
			return null
		unique_targets[target_state] = true
	return result
...
## 朝向同步扩展点；带图片和攻击判定的角色薄层在此统一镜像相关节点。
...
func _sync_state_visual(state_name: StringName) -> void:
	...
	if state_name in [&"jump_start", &"jump", &"double_jump", &"fall", &"land", &"dash_forward", &"dash_back"]:
		...
		color_key = &"defense"
	elif state_name in [&"stand_light", &"stand_medium", &"stand_heavy", &"stand_combo_medium", &"crouch_light", &"crouch_medium", &"crouch_heavy", &"air_light", &"air_medium", &"air_heavy", &"projectile", &"enhanced_projectile"]:
		color_key = &"attack"
