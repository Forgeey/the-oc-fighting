...
func _ready_impl(context: Dictionary) -> void:
	...
	input_prefix = String(context.get("input_prefix", "p1_"))


## 使用当前 fighter 的输入前缀构造 Fray 语义输入名。
func input_name(suffix: String) -> StringName:
	return StringName(input_prefix + suffix)
