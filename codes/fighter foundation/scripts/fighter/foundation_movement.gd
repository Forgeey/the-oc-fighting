...
## 使用攻击资源启动受击击退；空中受击只在资源提供 launch 时改写垂直速度。
func start_hit_reaction(attribute: FrayAttackAttribute, airborne: bool) -> void:
	_jump_air_locked = false
	_fighter.velocity.x = -float(_fighter.facing_direction) * attribute.knockback
	if not is_zero_approx(attribute.launch_y_velocity):
		_fighter.velocity.y = attribute.launch_y_velocity
	elif not airborne and _fighter.velocity.y > 0.0:
		_fighter.velocity.y = 0.0
...
## 使用攻击资源启动倒地运动。
func start_knockdown(attribute: FrayAttackAttribute) -> void:
	start_hit_reaction(attribute, true)
...
func _apply_gravity(delta: float) -> void:
	if not _is_floor_contact() and not _is_at_or_below_environment_floor():
		...
		_fighter.velocity.y = minf(_fighter.velocity.y + gravity * delta, _environment.max_fall_speed)
...
func _is_floor_contact() -> bool:
	if _fighter.is_on_floor():
		return true
	return _fighter.global_position.y >= _environment.floor_y - FLOOR_CONTACT_EPSILON and _fighter.velocity.y >= 0.0
