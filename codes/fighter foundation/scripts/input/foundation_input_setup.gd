...
func configure_input_map(prefix: StringName, fighter) -> bool:
	...
	for suffix in ["left", "right", "down", "up", "light", "heavy", "guard"]:
...
## 创建 forward→forward 与 back→back 的 FraySequenceTree。
func create_dash_sequences(prefix: StringName) -> FraySequenceTree:
